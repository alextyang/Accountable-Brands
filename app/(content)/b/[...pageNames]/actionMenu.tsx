...
function ActionDropdown({pageName, styles=''}:{pageName:string, styles?:string}) {
    const items = [
        {title: (pageName.includes('/') ? 'Edit Report' : 'Edit Brand'), icon: 'edit-page', href: MW_DIRECT_URL+'?title='+pageName+'&veaction=edit', p: '0.5'},
        {title: 'Open Discussion', icon: 'discussion', href: MW_DIRECT_URL+'?title=Talk:'+pageName+'', p: '0.5'},
        // {title: 'Report Content', icon: 'report', href: MW_DIRECT_URL+'?title=Talk:'+pageName+'', p: '0.25'},
    ];

    return (
        <div className={styles+' b border-x-black border-x-6  flex flex-nowrap flex-col z-50 font-medium -pt-1.5 whitespace-nowrap text-base'}>
            <div className=" text-tan bg-black px-1 pt-1 pb-0.5">
                <p>Content is user-submitted.</p>
            </div>
            {items.map((item) => {
                return (
                    <div key={item.title} className='flex flex-row h-12 cursor-pointer relative bg-tan -pb-1.5 items-center justify-start pr-4  border-b-black border-b-6'>
                        <Icon name={item.icon} styles={'aspect-square h-9 aspect-square m-1 mr-1 p-'+item.p}/>
                        <p>{item.title}</p>
