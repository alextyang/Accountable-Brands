...
export function ReportGrid ({reports}:{reports:ReportPage[]}) {
    ...
    const [openReport, setOpenReport] = useState(reports.map(report => report.title).indexOf(brandName+'/'+reportName)); // Index of currently open modal, -1 for none
    const [shouldAnimate, setShouldAnimate] = useState(openReport == -1 ? true : false);
   ...
   useEffect(() => {
        function navHandler(ev: PopStateEvent) {
            ...
            const newReportIndex = reports.map(report => report.title).indexOf(newReportName);

            if (newReportIndex != openReport) {
                console.log('[Page] URL changed to ', window.location.pathname);
                console.log('[Page] Switching index from '+openReport+' to ', newReportIndex);
                setTimeout( () => {
    ...
    const setReport = (index:number) => {
        ...
        console.log('[Grid] '+((index != -1) ? 'Opening' : 'Closing')+' report ',index);
        ...
        <div className="h-full">
            <div ref={gridRef} className='flex flex-row flex-wrap h-full justify-stretch relative -m-2 mt-1'> {/* Report Grid */}
                ...
                <div className={shouldAnimate ? "" : "hidden"}>
                    <Transition in={(openReport != -1)} timeout={duration} onEntering={() => saveNavigation()} onExiting={() => saveNavigation()} >
                        {state =>
                            <div>
                                <AnimatedReportModal duration={duration} state={state} report={reports[openReport]}  setOpenReport={setReport} modalOffsetBox={modalOffsetBox} modalOriginBox={modalOriginBox}/>
                                {reports[openReport] ? ( 
                                    <div className="">
                                        <Icon name="close" styles="z-20 !block fixed top-2 right-4 xl:top-6 xl:right-6 w-14 h-14 pointer-events-none" color="rgb(216 193 172 / 0.8)" />
                                    </div>
                                ): ''} 
                                {reports[openReport] ? ( 
                                    <div className=" opacity-0 z-10 fixed top-0 bottom-0 left-0 right-0 cursor-pointer " onClick={() => setReport(-1)}>
                                    </div>
                                ): ''} 
                            </div>
                        }
                ...
                <div className={shouldAnimate && openReport != -1 ? "hidden" : ""}>
                    <StaticReportModal report={reports[openReport]}  setOpenReport={(i:number) => {setReport(i);}} />
                    {reports[openReport] ? ( 
                        <div className="">
                            <Icon name="close" styles="z-20 !block fixed top-2 right-4 xl:top-6 xl:right-6 w-14 h-14 pointer-events-none" color="rgb(216 193 172 / 0.8)" />
                        </div>
...
function ReportItem ({report, index, openReport, setOpenReport}:{report:ReportPage|undefined,openReport:number,index:number,setOpenReport:Function}, ref:ForwardedRef<HTMLDivElement>) {
    ...
    return (
            <div ref={ref} className={"relative h-44 sm:h-40 md:h-44 lg:h-52 2xl:h-60 3xl:h-56 basis-full lg:basis-1/2 2xl:basis-1/3  " + ((openReport != -1)?'-z-10 pointer-events-none':'z-10 pointer-events-auto')}> {/* Flex Item */}
                <a className={"z-10 absolute top-0 bottom-0 right-0 left-0 opacity-0 hover:opacity-100 cursor-pointer "+(isThisReportOpen ? 'opacity-0 ' : '')} onClick={(e) => {setOpenReport(index)}}> {/* Link Overlay */}
                    <Icon name="expand" styles="absolute right-6 bottom-6 h-14 w-14" />
                </a>
                ...
                <div className={"absolute top-0 bottom-0 right-0 left-0 m-2 border-black border-6 bg-tan/60"}> {/* Visual Box */}
                    <div className={"absolute top-0 bottom-2 sm:bottom-3 md:bottom-2 right-0 left-0 pl-3 pr-2.5 py-2.5 overflow-hidden"}> {/* Text cut off */}
                        <div className={"float-right relative ml-32 w-14 h-14 p-1 "+REPORT_TYPES[report.type].color}> {/* Type Icon & Labels */}
                            <Icon styles={REPORT_TYPES[report.type].iconStyle} name={REPORT_TYPES[report.type].icon} color="#D8C1AC" />
                            <div className="absolute right-16 top-0 text-right "> {/* Date & Type Label */}
                        ...
                        <div className="-mt-0"><p className="text-2xl font-medium leading-7 tracking-tight text-balance">{report.title.substring(report.title.indexOf('/')+1)}</p></div> 
                            {/* Title */}
                        <p className="text-base mt-1 ml-0.5 mr-20 line-clamp-4 sm:line-clamp-3 md:line-clamp-4 xl:line-clamp-5">{report.preview}</p> {/* Content Preview */}
                    </div>
                </div>
            </div>
    );
...
function AnimatedReportModal({duration, state, report, setOpenReport, modalOriginBox, modalOffsetBox}:{duration:number,state:TransitionStatus,report:ReportPage|undefined,setOpenReport:Function,modalOriginBox:DOMRect|undefined,modalOffsetBox:DOMRect|undefined}) {
    ...
    const origin = {
        ...
        width: modalOriginBox.width,backgroundColor: 'rgba(7, 9, 15, 0)',opacity:'0'};

    return (
        <div style={{transition: `left 300ms ease-out, top 300ms ease-out, width 300ms ease-out, height 300ms ease-out, background 400ms ease-in-out`,
            transitionDelay:'0ms,0ms,0ms,0ms,300ms',
            ...origin, ...{
            entering:origin,
            exited:origin,
            entered:{top:'0px',height:'100%',left:'0px',width:'100%',opacity:'100%',backgroundColor: 'rgba(7, 9, 15, 0.4)'},
            exiting:{...origin}, 
            unmounted:origin
            }[state]}} className={'fixed z-20 w-full h-full flex flex-col items-center justify-center content-center pointer-events-none'}>
                

                {report ? (
                    <div
                    style={{transition: `background-color 10ms ease-out 0ms, max-height 1000ms ease-in-out 300ms, margin-top 300ms`, maxWidth:'1180px', marginTop:'72px', ...{
                            entering:{backgroundColor: "rgb(216 193 172 / 0.0)", flexGrow: '10'},
                            exited:{},
                            entered:{backgroundColor: "rgb(216 193 172 / 1)", flexGrow: '0.001', maxHeight:'100%'},
                            exiting:{}, 
                            unmounted:{}
                        }[state]}} className={"report-container opacity-100 relative pointer-events-auto overflow-hidden z-30 m-2 pl-3 pr-2.5 py-2.5  border-black border-6 transition-colors max-h-44 sm:max-h-40 md:max-h-44 lg:max-h-52 2xl:max-h-60 3xl:max-h-56 bg-tan/60 xl:!mt-2"}> {/* Visual Box */}

                       
                        <div className="flex flex-col float-right">
                            <div style={{transition: `margin-left 300ms,height 600ms ease-in-out 680ms`, ...{
                                entering:{},
                                exited:{},
                                ...
                                exiting:{}, 
                                unmounted:{} 
                            }[state]}} className="absolute z-20 right-16 top-0 text-right opacity-100 "> {/* Date & Type Label */}
                                    <p className="text-sm whitespace-nowrap font-medium  ">{report.timeframe}</p>
                                    <p className={"text-sm whitespace-nowrap font-medium -mt-1 "+REPORT_TYPES[report.type].text}>{REPORT_TYPES[report.type].name}</p>
                                </div>
                        ...
                        <div style={{transition: ``, ...{
                            ...
                            <p style={{transition: `transform 350ms ease-in-out, margin-bottom 350ms ease-in-out, padding 350ms ease-in-out, opacity 0ms ease-in 800ms `, transformOrigin:'top left', ...{
                                entering:{transform:'scale(1)',marginBottom:'0px',padding:'0 0 0 0'},
                                exited:{},
                                entered:{transform:'scale(1.5)',marginBottom:'0px',padding:'0.1rem 40% 0.5rem 0.25rem',opacity:'0'},
                                exiting:{}, 
                                unmounted:{}
                            }[state]}} className="h-min -mt-0 text-2xl font-medium leading-7 absolute tracking-tight text-balance">{report.title.substring(report.title.indexOf('/')+1)}</p>
                        </div> 

                        {/* White->Black Title  */}
                        <div style={{transition: `clip-path 600ms ease-in-out 350ms`, top:'10.5px', marginRight:'4.7rem', lineHeight:'2.625rem', ...{
                        ...
                        {/* Spacer Title  */}
                        <p style={{top:'10.5px', paddingRight:'4.7rem', lineHeight:'2.625rem'}} className="h-min -mt-0 w-full opacity-0 pointer-events-none p-1 pl-2 text-4xl font-medium tracking-tight text-balance">{report.title.substring(report.title.indexOf('/')+1)}</p>


                            
                        {/* Fading out Preview  */}
                        <p style={{transition: `opacity 250ms ease-in-out 0ms, max-height 0ms ease-out 400ms`, ...{
                            entering:{opacity:'100%'},
                            exited:{},
                            entered:{opacity:'0%', maxHeight:'0%'},
                            exiting:{}, 
                            unmounted:{}
                        }[state]}} className="text-base absolute mt-1 ml-0.5 mr-20 max-h-full line-clamp-4 sm:line-clamp-3 md:line-clamp-4 xl:line-clamp-5">{report.preview}</p>

                        {/* Fading in Content  */}
                        <div style={{transition: `opacity 300ms ease-in-out 250ms`, ...{
                            entering:{opacity:'0%'},
                            exited:{},
                            entered:{opacity:'100%'},
                            exiting:{}, 
...
function StaticReportModal({report, setOpenReport}:{report:ReportPage|undefined,setOpenReport:Function}) {
    if (!report) return '';


    return (
        <div style={{top:'0px',height:'100%',left:'0px',width:'100%',opacity:'100%',backgroundColor: 'rgba(7, 9, 15, 0.4)'}} className={'fixed z-20 w-full h-full flex flex-col items-center justify-center content-center pointer-events-none'}>
                

                {report ? (
                    <div
                    style={{ maxWidth:'1180px', marginTop:'72px',backgroundColor: "rgb(216 193 172 / 1)", flexGrow: '0.001', maxHeight:'100%'}} className={"report-container opacity-100 relative pointer-events-auto overflow-hidden z-30 m-2 pl-3 pr-2.5 py-2.5  border-black border-6 transition-colors max-h-44 sm:max-h-40 md:max-h-44 lg:max-h-52 2xl:max-h-60 3xl:max-h-56 bg-tan/60 xl:!mt-2"}> {/* Visual Box */}
